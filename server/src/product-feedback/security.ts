...
function contactAad(submissionId: string): Buffer {
  return Buffer.from(`paperclip-product-feedback:reporter-email:v1:${submissionId}`, "utf8");
}

export function encryptContact(value: string, key: Buffer, submissionId: string): string {
  if (key.length !== 32) throw new Error("contact encryption key must contain exactly 32 bytes");
  const iv = randomBytes(12);
  const cipher = createCipheriv("aes-256-gcm", key, iv);
  cipher.setAAD(contactAad(submissionId));
  const ciphertext = Buffer.concat([cipher.update(value, "utf8"), cipher.final()]);
  return `v1.${iv.toString("base64url")}.${cipher.getAuthTag().toString("base64url")}.${ciphertext.toString("base64url")}`;
}

export function decryptContact(value: string, key: Buffer, submissionId: string): string {
  ...
  decipher.setAAD(contactAad(submissionId));
