...
export const asanaWebhookEnvelopeSchema = z.object({
  events: z.array(z.object({
    action: z.string().min(1).max(50),
    resource: z.object({
      gid: z.string().min(1).max(100),
      resource_type: z.string().min(1).max(100),
      resource_subtype: z.string().max(100).optional(),
    }).passthrough(),
    parent: z.object({
      gid: z.string().min(1).max(100),
      resource_type: z.string().min(1).max(100),
...
export type AsanaWebhookEnvelope = z.infer<typeof asanaWebhookEnvelopeSchema>;
