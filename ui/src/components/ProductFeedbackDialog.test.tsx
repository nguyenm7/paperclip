...
describe("ProductFeedbackDialog", () => {
  ...
  it("defaults follow-up on and confirms a known authenticated email", async () => {
    ...
    expect(captureEvent).toHaveBeenCalledWith("survey shown", expect.objectContaining({
      deploymentMode: "authenticated",
    }));
  });
  ...
  it("submits a direct local validation without collecting contact data or requesting a grant", async () => {
    ...
    expect(document.body.textContent).toContain("does not collect contact details");

    await setValue(document.body.querySelector("textarea") as HTMLTextAreaElement, "Direct canary feedback");
    await act(() => findButton("Send feedback")?.click());
    await flush(3);

    expect(requestGrant).not.toHaveBeenCalled();
    expect(captureEvent).toHaveBeenCalledWith("survey sent", expect.objectContaining({
      feedback: "Direct canary feedback",
      followUpConsent: true,
    }));
    expect(JSON.stringify(captureEvent.mock.calls)).not.toContain("owner@example.com");
    expect(document.body.textContent).toContain("Feedback sent");
  });
});
