...
import { productFeedbackDraftKey, useProductFeedbackDraft } from "@/lib/product-feedback-draft";
...
interface ProductFeedbackDialogProps {
  ...
  /** Authenticated session user ID. Never pass an email address. */
...
export function ProductFeedbackDialog({
  ...
  // Draft scope: session user ID in authenticated mode, a fixed board scope in
  // local_trusted mode. The key builder also rejects email-like values so
  // contact data cannot become a storage key if this prop is wired incorrectly.
  const draftScope = deploymentMode === "authenticated"
    ? authenticatedUserId
    : "local-board";
  const draftKey = posthog && draftScope
    ? productFeedbackDraftKey(draftScope, posthog.surveyId)
    : null;
  const { feedback, setFeedback, clearStoredDraft } = useProductFeedbackDraft(draftKey, maxLength);
  ...
  const shownForOpenRef = useRef(false);
  ...
  const emailInvalid = !directDelivery && followUpConsent && !validEmail(selectedEmail);
  ...
  useEffect(() => {
    if (!open || shownForOpenRef.current || !capability.enabled || !posthog) return;
    shownForOpenRef.current = true;
    void capture("survey shown", eventContext()).catch(() => {
        recordProductFeedbackDiagnostic({ code: "survey_shown_capture_failed", component: "feedback_dialog" });
      });
  }, [capability, deploymentMode, appVersion, open, posthog, captureEvent]);
  ...
  function handleOpenChange(nextOpen: boolean) {
    ...
    if (status !== "success" && shownForOpenRef.current && capability.enabled && posthog) {
      void capture("survey dismissed", eventContext()).catch(() => {
          recordProductFeedbackDiagnostic({ code: "survey_dismissed_capture_failed", component: "feedback_dialog" });
        });
    }
    shownForOpenRef.current = false;
    if (status === "success") resetForNextSubmission();
    onOpenChange(false);
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate aria-busy={submitting}>
            ...
            <div className="space-y-3">
              ...
              {directDelivery ? (
                <p className="text-xs text-muted-foreground">
                  This local validation records your preference but does not collect contact details.
                </p>
              ) : null}

              {!directDelivery && followUpConsent && accountEmail && !changeEmail ? (
            ...
            <details className="group rounded-md bg-muted/60 px-3 py-3 text-sm text-muted-foreground">
              ...
              <ul className="mt-3 list-disc space-y-1 pl-5">
                ...
                <li>
                  {directDelivery
                    ? "A random submission ID, your follow-up preference, and the local validation run ID"
                    : "A random submission ID, short-lived contact grant, and anonymous installation reference"}
                </li>
              </ul>
            </details>
            ...
            <p className="text-xs leading-relaxed text-muted-foreground">
              {directDelivery
                ? "This local validation sends your feedback, follow-up preference, and the diagnostic details listed here to PostHog. It does not send your email."
                : "Submitting sends this feedback and the diagnostic details listed here to Paperclip's feedback systems. Your email is sent through a separate contact grant and is never sent to PostHog."}
            </p>
