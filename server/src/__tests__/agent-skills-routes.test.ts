...
describe.sequential("agent skill routes", () => {
  ...
  it("rejects skill sync by default under the narrow agent_config:update grant", async () => {
    mockAgentService.getById.mockResolvedValue(makeAgent("claude_local"));
    mockAccessService.decide.mockResolvedValue({
      allowed: true,
      reason: "allow_explicit_grant",
  ...
  it("preserves skill sync when agents:create authorizes the request", async () => {
    mockAgentService.getById.mockResolvedValue(makeAgent("claude_local"));
    mockAccessService.decide.mockResolvedValue({
      allowed: true,
      reason: "allow_explicit_grant",
