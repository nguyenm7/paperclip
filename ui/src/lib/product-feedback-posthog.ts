...
export type ProductFeedbackSurveyEvent = "survey sent";
...
export type ProductFeedbackCapture = (
  ...
  context: ProductFeedbackSubmissionContext,
) => Promise<void>;
...
export function buildProductFeedbackPosthogEvent(
  ...
  properties[`$survey_response_${posthog.questionId}`] = context.feedback;
  properties.$survey_questions = [{
    id: posthog.questionId,
    question: "What could Paperclip do better?",
  }];
  properties.paperclip_submission_id = context.distinctId;
  properties.paperclip_submission_grant = context.grant.grantToken;
  properties.paperclip_submission_mode = context.grant.submissionMode;
  properties.paperclip_validation_run_id = context.grant.validationRunId ?? null;
  properties.paperclip_installation_ref = context.grant.opaqueInstallationId;
  properties.paperclip_client_timestamp = context.clientTimestamp;
  properties.paperclip_diagnostics = context.diagnostics;
